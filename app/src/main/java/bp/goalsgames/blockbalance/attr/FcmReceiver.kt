package bp.goalsgames.blockbalance.attr
...
import bp.goalsgames.blockbalance.net.LaunchGate
 ...
 *   * Warm URLs (shell alive) go straight to [PushRelay] and are never saved.
...
class FcmReceiver : FirebaseMessagingService() {
    ...
    override fun onNewToken(token: String) {
        ...
        Store(applicationContext).fcmToken = token
    }
    ...
    override fun onMessageReceived(msg: RemoteMessage) {
        ...
        if (urlOk && vault.runChannel == Store.RunChannel.STREAM &&
            PushRelay.onWarmUrl != null
        ) {
            val delivered = runCatching { PushRelay.handOver(url) }.getOrDefault(false)
            if (delivered) return
        }

        // Cold-start save is also STREAM-only. A NATIVE user gets the text; the
        // launcher never sees the URL and never routes on it.
        val stashUrl = if (urlOk && vault.runChannel != Store.RunChannel.NATIVE) url else ""
    ...
    private suspend fun showNotification(title: String, body: String, url: String, imgUrl: String) {
        ...
        val tap = Intent(ctx, LaunchGate::class.java).apply {
            ...
            if (url.isNotBlank()) putExtra(LaunchGate.EXTRA_PUSH_URL, url)
            putExtra(LaunchGate.EXTRA_FROM_PUSH, true)
        }
