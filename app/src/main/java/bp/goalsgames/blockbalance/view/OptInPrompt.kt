...
class OptInPrompt : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val btnW = if (stacked) 330 else 165
        val acceptBtn = buildButton("ACCEPT", accent = true, widthDp = btnW)
        val skipBtn   = buildButton("SKIP",   accent = false, widthDp = btnW)

        acceptBtn.setOnClickListener { onAccept() }
        skipBtn.setOnClickListener   { onSkip()   }

        btnRow.addView(acceptBtn)
        btnRow.addView(space(if (stacked) 12 else 16, stacked))
    ...
    private fun buildButton(label: String, accent: Boolean, widthDp: Int): TextView {
        ...
        val lp = LinearLayout.LayoutParams(dpToPx(widthDp), dpToPx(54))
        tv.layoutParams = lp
        return tv
    }

    private fun space(dp: Int, vertical: Boolean = false): View = View(this).apply {
