...
class KeyboardSlide(private val host: View, private val vault: Store) {
    ...
    private var layoutWatching = false

    private val probe = Runnable {
        val view = web ?: return@Runnable
        val name = BuildConfig.JS_KEYBOARD_SENTINEL + "Report"
        view.evaluateJavascript(script) {
            view.evaluateJavascript("window.$name && window.$name();", null)
        }
    }
    ...
    fun install() {
        ...
        ViewCompat.setWindowInsetsAnimationCallback(
            ...
            object : WindowInsetsAnimationCompat.Callback(DISPATCH_MODE_STOP) {
                ...
                override fun onEnd(animation: WindowInsetsAnimationCompat) {
                    ...
                    if (keyboard > 0) poke() else apply(animated = false)
                }
            }
        )
        ...
        // Belt and braces: some windows never deliver IME insets to decorView.
        // Root insets on a layout pass still see the keyboard.
        if (!layoutWatching) {
            layoutWatching = true
            host.viewTreeObserver.addOnGlobalLayoutListener {
                val ime = ViewCompat.getRootWindowInsets(host)
                    ?.getInsets(WindowInsetsCompat.Type.ime())?.bottom ?: 0
                if (!riding && ime != keyboard) {
                    settle(ime)
                    apply(animated = keyboard > 0)
                    if (keyboard > 0) poke()
                }
    ...
    /** JavascriptInterface must exist before the page loads. */
    ...
    private fun poke() {
        host.removeCallbacks(probe)
        host.post(probe)
        host.postDelayed(probe, SETTLE_MS)
    }
    ...
    val script: String by lazy(LazyThreadSafetyMode.PUBLICATION) {
        ...
        (function(){
          ...
          function pierce(el){
            // Custom elements hide the real field in a shadow root; the host is
            // what activeElement points at, so walk down to the focused node.
            var depth = 0;
            while (el && el.shadowRoot && el.shadowRoot.activeElement && depth++ < 6) {
          ...
          function locate(){
            var el = pierce(document.activeElement);
            var win = window;
            var offset = 0;
            var depth = 0;
            while (el && (el.tagName === 'IFRAME' || el.tagName === 'FRAME') && depth++ < 4) {
              ...
              var inner = doc ? pierce(doc.activeElement) : null;
          ...
          function watch(doc){
            try {
              ...
              doc.addEventListener('focus', kick, true);
            } catch(e) {}
          }
